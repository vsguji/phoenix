...
class BrnIntl {
  /// 内置支持的语言和资源
  final Map<String, BaseBaseResource> _defaultResourceMap = {
    'en': BaseResourceEn(),
    'zh': BaseResourceZh()
  };
  ...
  static BaseBaseResource i10n(BuildContext context) {
  ...
  /// 支持非内置的本地化能力
  static void add(Locale locale, BaseBaseResource resource) {
    _BrnIntlHelper.add(locale, resource);
  }

  /// 支持非内置的本地化能力
  static void addAll(Locale locale, List<BaseBaseResource> resources) {
...
class _BrnIntlHelper {
  ...
  static void addAll(Locale locale, List<BaseBaseResource> resources) {
    ...
    for (BaseBaseResource resource in resources) {
